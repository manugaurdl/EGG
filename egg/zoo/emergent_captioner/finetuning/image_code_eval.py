...
def main(params, config):
    ...
    name2wrapper = {
        ...
        "image_code": ImageCodeWrapper,
    }
    # args
    wrapper = name2wrapper[opts.train_dataset](captions_type = config["captions_type"], dataset_dir = opts.dataset_dir,jatayu = opts.jatayu, neg_mining = config["neg_mining"], image_dir= image_dir)
    ...
    test_loader = wrapper.get_split(split="valid", transform = get_transform(opts.sender_image_size, opts.recv_image_size), batch_size = opts.batch_size)
    val_loader_neg = None
