...
def get_loss(
    temperature: float = 1.0, similarity: str = "cosine", loss_type: str = "comm_ntxent"
):
...
class XEntLoss(Loss):
    def xent_loss(
        self, message: torch.Tensor, receiver_output: torch.Tensor, labels: torch.Tensor
    ):
        model_guesses = torch.bmm(receiver_output, message.unsqueeze(-1)).squeeze(-1)
    ...
    def __call__(
        self, _sender_input, message, _receiver_input, receiver_output, labels
    ):
...
class CommNTXentLoss(Loss):
    def comm_nt_xent_loss(self, message: torch.Tensor, receiver_output: torch.Tensor):
        ...
        model_guesses = torch.argmax(
            torch.cat((logits_msg_img, logits_img_msg), dim=0), dim=1
        )
    ...
    def __call__(
        ...
        assert not (
            message.shape[0] % 2
        ), f"batch must be multiple of 2, found {message.shape[0]} instead"
